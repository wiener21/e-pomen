...
export default async function Obituary({article, content}) {
    ...
    async function getPerson(id) {
        ...
        const res = await fetch(process.env.ADMIN_URL+'/wp-json/wp/v2/person/'+id,{mode: 'no-cors'});
    ...
    async function getCemetery(id) {
        ...
        const res = await fetch(process.env.ADMIN_URL+'/wp-json/wp/v2/cemetery/'+id,{mode: 'no-cors'});
        // The return value is *not* serialized
        // You can return Date, Map, Set, etc.
        // Recommendation: handle errors
        if (!res.ok) {
